...
class JTProduct extends MongoBase {
    ...
    public function getProducts(){
        ...
        $arrs = $this->find([
            ...
            ,'fields' => ['_id', 'name', 'sku', 'code', 'image'],
            'sort'   => ['name' => 1]
        ]);
        foreach ($arrs as $arr){
            $arr_return[] = $arr;
    ...
    public function getNameById($id=''){
        ...
        $option_product = $product->options;
        foreach ($option_product as $k2 => $v2) {
            if($v2['deleted']==true)
                    continue;
                  if($v2['require']==1 && isset($v2['product_id']) && isset($v2['quantity'])){
                $option_product[$k2]['quantity'] = (string) $v2['require'];
                $option_product[$k2]['_id'] = (string) $v2['product_id'];
            }
        }

        $total_price = JTProduct::getPrice([
        ...
        $sell_price  = $total_price['sell_price'];

        $product->image = '';
        if(isset($product->products_upload)){
            $product->image = $product->products_upload;
        ...
        $arr_return = array(
            'image' => (isset($product->image) && $product->image!='' ? $product->image : "/themes/pizzahut/images/default.png")
            ,'name'=>(isset($product->name)?$product->name:"")
            ,'description'=>(isset($product->description)?$product->description:"")
            ,'product_desciption'=>(isset($product->product_desciption)?$product->product_desciption:"")
            ,'custom'=>(isset($product->custom)?$product->custom:0)
            ,'price'=>(isset($product->price)?$product->price:0)
            ,'combo'=>(isset($product->combo)?$product->combo:0)
            ,'use_group_order'=>(isset($product->use_group_order)?$product->use_group_order:0)
            ,'combo_sales'=>(isset($product->combo_sales)?$product->combo_sales:0)
    ...
    public function getLists()
    {
        ...
        $categories = $JTCategory->findFirst(array(
            'conditions'=>array('setting_value'=>'product_category')
        ));
        $arr_list_category = [] ;
        $arr_list = $categories->toArray();
        $arr_list = $arr_list['option'];
        for($i=0;$i<count($arr_list);$i++){
            $arr_list_category [] = $arr_list[$i]['name'];
        }        
        ...
        $products = $this->find([
                 [
                    ...
                    'group_type'=>'SELL',
                    'category'=>array('$in'=>$arr_list_category)
                ],
        ...
        if ($products) {
            foreach ($products as $product) {
                ...
                $arrReturn[] = ['text' => 'Category :'.$product->category . ' --- ' .$product->name, 'value' => (string) $product->_id];
            }
        }
        return $arrReturn;
    }
    ...
    public static function getPrice($data)
    {
        ...
        $product = self::findFirst([
                ...
                'fields' => ['_id', 'name', 'description', 'sell_by', 'sell_price','options', 'pricebreaks', 'sellprices', 'pricing_method', 'category']
            ]);
        if( is_object($product) ) {            
            // $product = $product->toArray();
            $product = return_to_array($product);
            $product['sizew'] = $sizeW;
            $product['sizeh'] = $sizeH;
