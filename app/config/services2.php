...
$di->setShared('view', function () use ($config) {
    ...
    $view->registerEngines(array(
        '.volt' => function ($view, $di) use ($config) {
            ...
            $Compiler->addFunction('removespace','trim');
            $Compiler->addFunction('bospace','remove_spaces');
            $Compiler->addFunction('pr','pr');
            $Compiler->addFunction('jsd','json_en_de');
...
$di->set('crypt', function() {
    ...
    $crypt->setKey('1234@1234@');
    return $crypt;
});
//for send email
$di->set('mailer',function() use ($config) {
    include '../vendor/swiftmailer/swiftmailer/lib/swift_required.php';
    $transport = Swift_SmtpTransport::newInstance($config->smtp->host,$config->smtp->port,$config->smtp->encryption)
