    ...
    if (['add', 'edit'].indexOf(action) != -1) {
        $scope.form = {
            fields: [{
                ...
                label: 'Type',
                name: 'type',
                options: [{value:'pages',text:'Page'},{value:'users',text:'User'},{value:'faqs',text:'FAQs'}],
                value :'pages'
            }, {
                type: 'select',
                label: 'Category',
        ...
        if (action === 'edit') {
            ...
            $http.get(appHelper.adminURL('pages/edit/' + id))
                .success(function(result) {
                    if (result.error === 0) {
                        $scope.form.fields[3].options = result.data.categoryOptions;
            ...
            $http.get(appHelper.adminURL('categories/get-options'))
                    .success(function(result) {
                        if (result.error === 0) {
                            $scope.form.fields[3].options = result.data;
                        }
                        $scope.form.allLoaded = true;
                    });
        }
